...
def pronouns():
    if request.method == 'POST':
        male_pronoun = request.form['male']
        female_pronoun = request.form['female']
        return f'male pronouns = {male_pronoun} - female pronouns = {female_pronoun}'
        # Here you can process the form data and return a response
        return jsonify({'male': male_pronoun, 'female': female_pronoun})
    else:
